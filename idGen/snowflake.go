...
func NewSnowflake(datacenterid, workerid int64) (IDGenerator, error) {
	if datacenterid < 0 || datacenterid > datacenteridMax {
		return nil, errno.Errorf("datacenterid must be between 0 and %v", datacenteridMax-1)
	}
	if workerid < 0 || workerid > workeridMax {
		return nil, errno.Errorf("workerid must be between 0 and %d", workeridMax-1)
	}
...
func (s *Snowflake) NextID() (int64, error) {
	...
	if t > timestampMax {
		s.Unlock()
		return 0, errno.Errorf("epoch must be between 0 and %d", timestampMax-1)
	}
	s.timestamp = now
...
func GetGenTime(sid int64, layout string) (t string) {
	...
	t = time.Unix(GetGenTimestamp(sid)/1000, 0).Format(layout)
	return
}
